...
A recent multi-cohort study integrating 15 public AD microbiome projects developed machine learning classifiers across geographic populations (Wang et al., 2026), confirming that multi-population integration remains a methodological frontier. The present study is complementary, providing a systematic quantification of cross-cohort generalization failure and a mechanistic analysis of why batch correction methods fail to bridge the gap—questions that remain open even where multi-cohort classifiers have been attempted.
...
To characterize the mechanistic basis of generalization failure, we computed out-of-fold SHAP values for within-cohort classifiers and LOCO-trained classifiers (Figure 6). Using an OOF design avoids the in-sample overfitting bias identified in prior SHAP analyses of microbiome classifiers, where training-set SHAP values can reflect model memorization rather than genuine feature importance.
...
**Cohort-specific top predictors.** The most discriminative genus within each cohort was highly variable: *Subdoligranulum* (CN-associated, Zhuang 2018, logistic regression) versus *Akkermansia* (CN-associated, Ling 2020, logistic regression) versus *Streptococcus* (CN-associated, Zhu 2022, logistic regression) versus *Akkermansia* (CN-associated, Kazakhstan, logistic regression). For LightGBM, the top taxon per cohort was entirely distinct: *Lachnoclostridium* (Zhuang 2018), *Akkermansia* (Ling 2020), *Bacteroides* (Zhu 2022), *Castellaniella* (Kazakhstan) (Figure 6A; Supplementary Figure S2).
...
These results have direct implications for the microbiome harmonization literature. When the batch effect dominates by a factor of ~12, no linear correction method—however sophisticated—can reliably separate the two sources of variance. The field may need fundamentally different approaches, such as multi-cohort training with cohort as a stratification variable, domain adaptation methods, or meta-analytic frameworks that explicitly model between-cohort heterogeneity rather than attempting to remove it.
...
The critical implication of directional flips is that they reflect genuine between-population differences in the microbiome–disease relationship, not technical artifacts of sequencing. Standard batch correction methods cannot resolve directional disagreements because those disagreements are not technical noise—they are real biological differences in the meaning of a taxon's abundance across populations.
...
**Batch correction leakage in LOCO evaluation:** As noted in Section 2.7, the batch correction analysis is transductive: test-cohort data and labels were included during correction parameter estimation. The corrected LOCO values represent an upper bound on what this transductive design could achieve, but cannot be extrapolated to a fully prospective deployment scenario.
...
Wang, Y., et al. (2026). Multi-cohort integration of gut microbiome data improves cross-geographic Alzheimer's disease classification. *mSystems*. https://doi.org/10.1128/msystems.01754-25
