...
## 8. Final directional stability screen result

Eight genera meet the descriptive coefficient-sign-stability screen, for logistic regression only: **Agathobacter, Bifidobacterium, Coprococcus, Dorea, Lactobacillus, NK4A214 group, Romboutsia, Ruminococcus gnavus group** (`results/tables/logreg_directional_flips_stable.csv`). This is reported throughout the manuscript as a descriptive screen, not a statistically confirmed set of contradictions (see §15, item 1, and Methods §2.8/Limitations §4.6 in the manuscript itself).

**Post-publication-consistency patch (same day):** an independent inspection of the pushed code found that `scripts/08_shap_taxa_comparison.py::plot_direction_dotplot()` — an auxiliary, non-manuscript figure generator — still used mean signed SHAP for its x-axis position and axis/title labels ("Mean SHAP value (positive → AD, negative → CN)", "Cross-Cohort SHAP Direction", "← CN-associated"/"AD-associated →"), even though its ★ flip marker had already been switched to the coefficient-stability definition in the prior commit. This was a genuine, correctly-identified gap: the star/marker logic had been fixed, but the plot's underlying axis semantics had not. Separately, `generate_manuscript_figures.py`'s actual Figure 6 (the one used in the manuscript PDF) had a display-pool mismatch between Panel A and Panel B that caused only 5 of the 8 screened genera to appear starred in Panel A. Both were resolved in this patch. Figure 6 was redesigned so Panel A shows feature importance only (no direction, no stars) and Panel B shows a coefficient-value grid (rows = the eight genera, columns = the four cohorts, cell text = fold-count stability annotation, bordered cells = ≥8/10 criterion met). `plot_direction_dotplot()` was deleted outright from `08_shap_taxa_comparison.py` (rather than patched) since its purpose is now fully superseded by Figure 6 Panel B's coefficient grid; its call site, its stale output filename in the outputs listing, and the stale `results/figures/shap_direction_dotplot_logreg.png` artifact itself (which still contained the old mean-SHAP axis labels baked into the image) were all removed. No mean-SHAP-as-direction visualization remains anywhere in the codebase or the manuscript pipeline. **While preparing the corrected Figure 6, a second, independent error was also found and fixed**: the *Lactobacillus* dietary-hypothesis narrative in §3.6/§4.4 had incorrectly described a clean Chinese-cohorts-versus-Kazakhstan split; the actual per-fold data (`logreg_coefficient_stability.csv`) shows *Lactobacillus* is stable-negative in Zhuang 2018 only and stable-positive in the other three cohorts. The manuscript was corrected to report this accurately, and *Romboutsia* — which does show the clean three-Chinese-cohorts-negative/Kazakhstan-positive pattern — was substituted as the population-level example.
...
1. **No multiple-comparisons correction on the stable directional-flip screen.** The coefficient-stability criterion (≥8/10 folds) is applied to 14 candidate genera (those in the top-20 SHAP ranking in ≥2 cohorts) without a permutation-based estimate of how many "stable flips" would arise by chance under a null where no genus truly differs in direction across cohorts. A rigorous fix would permute diagnosis labels within each cohort many times and refit the full nested-CV coefficient-stability pipeline per permutation — at the observed ~100 seconds per real run, a few hundred to a thousand permutations would take multiple hours, which was not run. This is disclosed in the manuscript itself (Methods §2.8, Limitations §4.6) as a property of the descriptive screen, not a statistically confirmed result; it does not affect the more clearly interpretable single-genus examples (*Romboutsia*, and the *Akkermansia* negative control) discussed narratively.
...
3. ~~Figure 6 Panel A / Panel B taxa-pool mismatch~~ — **RESOLVED** in the same-day publication-consistency patch referenced in §8's addendum above. Panel A no longer attempts to show direction or flip status at all (feature importance only); Panel B now displays all eight genera meeting the stability screen directly as a coefficient grid, so there is no longer a partial/inconsistent display of the screened taxa. This item is retained here, struck through, only to preserve the audit trail of what this report originally flagged as unresolved.
...
Having implemented all five fixes, we asked: what would a genuinely skeptical reviewer say next? The three items in §15 above are the most defensible remaining objections we could construct, and none was concealed. Beyond those three, we specifically checked for and did not find: (b) any remaining place where the Jaccard null does not match its observed statistic; (c) any remaining place where PERMANOVA is credited with explaining batch-correction behavior; (d) any stale headline number inconsistent with its source CSV; (e) any script that fails to compile or run, or that produces NaN/missing output.

**Correction to this section, made honestly rather than silently:** the original version of this self-review also claimed to have checked for and found no remaining place where mean signed SHAP is described as a direction statistic. That check was incomplete — an independent inspection of the pushed code found exactly such a case (`plot_direction_dotplot()` in `08_shap_taxa_comparison.py`, an auxiliary figure whose ★ marker had been fixed to use the coefficient-stability definition, but whose axis labels, title, and x-axis position still used mean signed SHAP), which was then fixed in a targeted follow-up patch (deleted outright, superseded by Figure 6 Panel B's coefficient grid; see the addendum to §8 above). We report this here rather than leaving the original, overstated claim standing: our own verification pass missed a real instance of the exact problem it was checking for, and it took an independent re-check to find it. All five original concerns are, in our assessment, now substantively and honestly addressed, with the batch-correction and Jaccard-null fixes each producing a genuine reversal of a previous claim, and the SHAP-direction fix requiring a second pass to be actually complete rather than merely declared complete.
